	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				hid = clid+"_"+i;
				var clickFunction = "jQuery('#"+$.jgrid.jqID($t.p.id)+"').jqGrid('groupingToggle','"+hid+"');return false;";
				icon = "<span style='cursor:pointer;' class='ui-icon "+pmrtl+"' onclick=\""+clickFunction+"\"></span>";
				try {
					gv = $t.formatter(hid, n.value, cp[n.idx], n.value );
				} catch (egv) {
					gv = n.value;
				}
				var title = "<span style='cursor:pointer;' onclick=\""+clickFunction+"\">"+$.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary)+"</span>";
				str += "<tr id=\""+hid+"\" role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+colspans+"\">"+icon+title+"</td></tr>";
