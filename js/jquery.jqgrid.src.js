...
(function ($) {
	...
	jgrid.extend({
		...
		rotateColumnHeaders: function (columnNameOrIndex, headerHeight) {
			return this.each(function () {
				var self = this, $th, p = self.p,
					isWebkit = /(AppleWebKit)[ \/]([\w.]+)/i.test(navigator.userAgent),
					colModel = p.colModel, cellLayout = p.cellLayout,
					cm, headDiv, iCol = isNaN(columnNameOrIndex) ?
						p.iColByName[columnNameOrIndex] :
						parseInt(columnNameOrIndex, 10);
				if (iCol >= 0) {
					$th = $(self.grid.headers[iCol].el);
					if (headerHeight !== undefined) {
						//$("thead:first tr.ui-jqgrid-labels th.ui-th-column").height(headerHeight);
						if ($th.height() < headerHeight) { // ??? $th.closest("tr").height() < headerHeight ???
							$th.height(headerHeight);
						}
					}
					headerHeight = $th.height(); //$("thead:first tr.ui-jqgrid-labels th.ui-th-column").height(); // trHead.height(); $(grid[0].grid.headers[iCol].el).height();
					cm = colModel[iCol];
					// we must set width of column header div BEFOR adding class "rotate" to
					// prevent text cutting based on the current column width
					headDiv = $th.children("div"); //$("th.ui-th-column:eq(" + iCol + ") div", trHead); // $(grid[0].grid.headers[iCol].el).children("div")
					if (!jgrid.msie || document.documentMode >= 9) { // jgrid.msiever() >= 9 || 
						headDiv.width(headerHeight)
									.addClass("ui-jqgrid-rotate")
									//.css("bottom", isWebkit? 7: 0)
									//.css("top", - (jgrid.cell_width ? cellLayout : 0) - 7)
									//.css("top", "-1em")
									.css("top", "") // remove top which could be set by setGroupHeaders
									.css("left", (cm.width - headerHeight) / 2 - (jgrid.cell_width ? cellLayout : 0));
						// $th.outerWidth() === cm.width
						// $th.outerHeight() === headerHeight
						// cellLayout === $th.outerWidth() - $th.width()  
					}
					else { // jgrid.msiever() <= 8 || 
						// Internet Explorer 6.0-8.0 or Internet Explorer 9.0 in compatibility mode
						headDiv.width(headerHeight).addClass("ui-jqgrid-rotateOldIE");
						if (document.documentMode <= 8) { // documentMode is important to test for IE compatibility mode
							$("span", headDiv).css("left", 0);
							headDiv.css({ left: cm.width / 2 - 8, bottom: headerHeight / 2 });
							headDiv.css("top", "") // remove top which could be set by setGroupHeaders
							//headDiv.css("top", (-headerHeight / 2) + "px");
							var newBottom = headDiv.outerHeight(),
								rowHight = $th.parent().height(),
								newTop = (rowHight - headDiv.outerHeight()) / 2;
							if (newBottom === 0) {
								newBottom = rowHight - 24;
							} else {
								newBottom -= newTop + 6;
							}
							headDiv.css("bottom", newBottom);
						} else {
							headDiv.css("left", 3);
						}
		...
		increaseColumnResizerHeight: function () {
			"use strict";
			// get the header row which contains
			var headerRow = this.closest("div.ui-jqgrid-view").find("table.ui-jqgrid-htable>thead>tr.ui-jqgrid-labels"),
			...
			headerRow.find("div.ui-jqgrid-sortable").each(function () {
				var ts = $(this), newTop = (rowHight - ts.outerHeight()) / 2;
				if (ts.hasClass("rotateOldIE")) { // "WIDTH: 130px; BOTTOM: 65px; LEFT: 4px"
					if (($.browser.msie && ieVer === "8.0") || document.documentMode === 8) { // documentMode is important to test for IE compatibility mode
						newBottom = ts.outerHeight();
						if (newBottom === 0) {
							newBottom = rowHight - 24;
						} else {
							newBottom -= newTop + 6;
						...
						//if (newBottom < 0) {
						//    newBottom = newBottom + (rowHight + ts.outerHeight());
						//}
						ts.css("bottom", newBottom);
					} else {
						ts.css("left", 3);
					}
				} else {
					ts.css("top", ts.hasClass("rotate") ? -newTop: newTop);
				}
			});
		}
	});
